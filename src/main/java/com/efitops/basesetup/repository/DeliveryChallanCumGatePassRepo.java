...
public interface DeliveryChallanCumGatePassRepo extends JpaRepository<DeliveryChallanCumGatePassVO, Long> {
	...
	@Query(value = """
			...
			LEFT JOIN delivery_challan_cum_gate_pass dcg
			    ON dcg.delivery_challan_cum_gate_pass_id = (
			        SELECT MAX(dcg2.delivery_challan_cum_gate_pass_id)
			        FROM delivery_challan_cum_gate_pass dcg2
			        WHERE dcg2.work_order_no = job.doc_id
			          AND dcg2.active = 1
			          AND dcg2.cancel = 0
			    )
			...
			WHERE job.doc_id = :jobOrderNo
			  ...
			  AND job.cancel = 0;
				        """, nativeQuery = true)
	Set<Object[]> getDeliveryChallanCumGatePassDetails(@Param("jobOrderNo") String jobOrderNo,
