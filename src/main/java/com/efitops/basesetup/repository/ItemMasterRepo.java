         ...
         @Query(value = """
        		...
        		@Query(value = """
        				SELECT i.item_id,
        				       i.item_code,
        				       i.item_description,
        				       pu.unit_id AS primary_unit,
        				       uu.unit_id AS purchase_unit
        				FROM item i
        				LEFT JOIN unitmaster pu
        				       ON i.primary_unit = pu.unitmaster_id
        				LEFT JOIN unitmaster uu
        				       ON i.purchase_unit = uu.unitmaster_id
        				WHERE i.active = 1
        				  AND i.cancel = 0
        				  AND i.org_id = :orgId
        				  AND i.branch = :branch
        	 ...
        	 @Query(value = """
        		    ...
        		    @Query(value = """
        		            ...
        		            WHERE im.cancel = 0
        		              ...
        		              AND im.org_id = :orgId
        		              AND im.branch = :branch
        		              AND UPPER(a.value_description) IN ('FG')
        		            ORDER BY im.item_code
        		            """, nativeQuery = true)
					List<Object[]> getFGItemsforBOMCorrectionRequestNote(Long orgId, Long branch);
					...
					@Query(value = """
        		            ...
        		            WHERE im.cancel = 0
        		              ...
        		              AND im.branch = :branch
        		              AND (
								    a.value_description IS NULL
								    OR UPPER(TRIM(a.value_description)) <> 'FG'
								)
        		            ORDER BY im.item_code
        		            """, nativeQuery = true)
					List<Object[]> getAllItemsNotFGforBOMCorrectionRequestNote(Long orgId, Long branch);}