...
@Repository
public interface OtherSalesInvoiceRepo extends JpaRepository<OtherSalesInvoiceVO, Long> {

	@Query(nativeQuery = true, value = "select * from other_sales_inv_basic where other_sales_inv_basic_id=?1 and active=1 and cancel=0")
	OtherSalesInvoiceVO getOtherSalesInvoiceById(Long id);

	@Query(nativeQuery = true, value = "select * from other_sales_inv_basic where org_id=?1  and branch=?2 and active=1 and cancel=0")
	List<OtherSalesInvoiceVO> getOtherSalesInvoiceByOrgId(Long orgId, Long branch);
