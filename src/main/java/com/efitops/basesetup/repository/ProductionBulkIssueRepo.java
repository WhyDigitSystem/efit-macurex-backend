...
public interface ProductionBulkIssueRepo extends JpaRepository<ProductionBulkIssueVO, Long> {
	...
	@Query(nativeQuery = true, value = "select doc_id,doc_date from material_indent_for_production   where org_id=?1 and \r\n"
			+ " branch=?2 and fg_item=?3 group by doc_id,doc_date")
	Set<Object[]> getIndentNoForProductionBulkIssue(Long orgId, Long branch,Long fgItem);
}
