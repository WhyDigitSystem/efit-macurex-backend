...
public interface QuotationRepo extends JpaRepository<QuotationVO, Long> {
	...
	@Query(nativeQuery = true, value = "select * from quotation_header where quotation_id=?1 and active=1 and cancel=0")
	QuotationVO getQuotationById(Long id);

	QuotationVO findByDocId(String docId);

	@Query(nativeQuery = true, value = "select * from quotation_header where org_id=?1  and branch=?2 and active=1 and cancel=0")
